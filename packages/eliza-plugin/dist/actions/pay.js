...
function hasPaymentRequiredSignal(input) {
...
async function getMcpClient() {
    if (!mcpClientPromise) {
        mcpClientPromise = (async () => {
            ...
            const client = new Client({ name: "eliza-crawlpay-client", version: "0.1.0" }, { capabilities: {} });
...
export const payForContentAction = {
    name: "PAY_FOR_CONTENT",
    description: "Detect CrawlPay 402 payment gates and initiate payment flow via MCP.",
    ...
    validate: async (_runtime, message, state) => {
        const messageText = stringifyUnknown(message.content);
    ...
    handler: async (_runtime, message, state, _options, callback) => {
        ...
        await callback?.({
            text: `Payment required for protected content.\n` +
                `URL: ${targetUrl}\n` +
                `Amount: ${amount}\n` +
                `\n${responseText}`,
        });
